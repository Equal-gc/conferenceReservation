...
Page({
  ...
  onLoad: function (options) {
    ...
    wx.request({
      ...
      success(res) {
        var collegeindex = 0;//学院的下标
        var majorindex = 0;//专业的下标
        var rs = res.data.data;
        for (var i = 0; i <that.data. multiArray[0].length;i++){
          if(rs.college==that.data.multiArray[0][i]){
            collegeindex = i;
          } 
        }
        var list = [];
        for (var j = 0; j < that.data.objectMultiArray.length; j++) {
          if (that.data.objectMultiArray[j].parid == that.data.objectMultiArray[collegeindex].regid) {
            list.push(that.data.objectMultiArray[j].regname)
          }
        }
        ...
        that.setData({
          "multiArray[1]": list,
          "multiIndex[0]": collegeindex,
          "multiIndex[1]": majorindex,
          userinfo: {
            openid: rs.openid,
            name: rs.name,
  ...
  clickButtonCommit() {
    ...
    wx.request({
      url: app.globalData.host + '/admin/personnel/updateUserInfo1',
      data: {
        userid: app.globalData.uid,
